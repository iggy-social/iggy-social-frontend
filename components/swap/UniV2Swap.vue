...
import { getOutputTokenAmount, getPriceImpactBps } from '@/utils/swap/uniV2'
...
export default {
  ...
  computed: {
    ...
    bothTokensAreNativeCoinOrWrappedTokenOrSame() {
      if (
        (this.inputToken.address == zeroAddress &&
        ...
        (this.inputToken.address == wrappedNativeTokens[this.$config.public.supportedChainId] &&
          this.outputToken.address == zeroAddress) ||
        (this.inputToken.address == zeroAddress &&
          this.outputToken.address == zeroAddress) ||
        (this.inputToken.address == wrappedNativeTokens[this.$config.public.supportedChainId] &&
          this.outputToken.address == wrappedNativeTokens[this.$config.public.supportedChainId])
    ...
    outputTokenAmount() {
      if (this.outputTokenAmountWei) {
        let amount = formatUnits(String(this.outputTokenAmountWei), this.outputToken.decimals)
  ...
  methods: {
      ...
      */
      ...
      if (this.inputTokenAmount) {
        ...
        if (this.bothTokensAreNativeCoinOrWrappedTokenOrSame) {
          this.outputTokenAmountWei = parseUnits(this.inputTokenAmount, this.inputToken.decimals)
        } else {
          ...
          // deduct slippage
          const slippageBps = this.siteSettings.getSlippageBps()
          this.outputTokenAmountWei = outputWei - (outputWei / BigInt(10000) * BigInt(slippageBps)) // apply slippage
        }
    ...
    async selectInputToken(token) {
      ...
      if (token.address == zeroAddress) {
        this.inputTokenAllowance = Number(maxUint256) // MaxUint256
      } else {
        this.inputTokenAllowance = await this.getTokenAllowance(token, this.address, this.routerAddress)
      }
    },
